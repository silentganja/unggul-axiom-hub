        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="space-y-3 pt-3 border-t border-border/10">
              ...
              <div className="space-y-2 max-h-[160px] overflow-y-auto pr-1">
                {historyTasks.map((task) => (
                  <div key={task.id} className="p-2.5 border border-border/10 rounded-sm bg-background/10 flex items-center justify-between text-[10px] select-all font-mono">
          ...
          <div className="lg:col-span-6 border border-border/30 rounded-sm bg-background-panel/20 backdrop-blur-sm p-5 space-y-4 flex flex-col justify-between">
            <div className="space-y-4">
              <div>
                <h2 className="text-sm font-bold tracking-tight text-foreground font-serif">Axiom Lock Registry & Override</h2>
                <p className="text-[10px] text-foreground-subtle font-mono mt-0.5">View and manage directory objects currently restricted by corporate blocks.</p>
              </div>
              <div className="space-y-2 max-h-[380px] overflow-y-auto pr-1">
                {lockedFiles.length === 0 ? (
                  <div className="py-16 text-center border border-dashed border-border/20 rounded bg-background/5 text-foreground-subtle">
                    ...
                    <p className="text-[10px] text-foreground-subtle mt-1 max-w-xs mx-auto leading-relaxed">All corporate repository assets are currently unrestricted.</p>
                  </div>
                ) : (
                  lockedFiles.map((file) => (
                    <div key={file.id} className="p-3 border border-border/25 rounded-sm bg-background-panel/40 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs">
                      <div className="space-y-1.5 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-mono text-[9px] text-foreground-subtle truncate max-w-[100px]">ID: {file.id.slice(0, 8)}</span>
                        </div>
                        <h4 onClick={() => setPreviewFileId(file.id)} className="font-bold text-foreground truncate hover:text-accent cursor-pointer max-w-[200px]" title="Preview secure file">{file.name}</h4>
                        <p className="text-[10px] text-foreground-subtle leading-tight font-mono">LOCKED BY: {file.lockedBy}<br />REASON: {file.lockReason || 'Pending Corporate Approval'}</p>
                      </div>
                      <button onClick={() => unlockFile(file.id)} className="h-7 px-3 rounded-sm border border-accent/30 text-accent bg-accent/5 hover:bg-accent/15 transition-all text-[9px] font-bold tracking-wider uppercase font-mono shrink-0 self-end sm:self-center cursor-pointer">Override Unlock</button>
                    </div>
                  ))
                )}
              </div>
            ...
            <div className="border-t border-border/10 pt-3 flex justify-between items-center text-[9px] font-mono text-foreground-subtle select-none mt-4">
              ...
              <span className="text-warning flex items-center gap-0.5"><Shield size={10} /> C-SUITE ONLY</span>
            </div>
          </div>
        </div>
      </div>
                        ...
                        {activeMenuId === file.id && (
                          <>
                            ...
                            <div className="absolute right-4 mt-1 z-30 w-44 rounded border border-border/80 bg-background-panel shadow-md p-1 space-y-0.5 text-left font-mono">
                              <button onClick={() => { if (file.type !== "folder") setPreviewFileId(file.id); else openAccessControl(file); setActiveMenuId(null); }} className="w-full flex items-center gap-2 px-2.5 py-1.5 text-[10px] text-foreground-muted hover:text-foreground hover:bg-background-subtle/50 rounded transition-colors">
                                <Eye size={11} className="text-foreground-subtle shrink-0" /> View Details
                              </button>
                              <button onClick={() => openAccessControl(file)} className="w-full flex items-center gap-2 px-2.5 py-1.5 text-[10px] text-foreground-muted hover:text-foreground hover:bg-background-subtle/50 rounded transition-colors">
                              ...
                              {file.lockedBy ? (
                                ...
                                <button onClick={() => initiateRename(file)} className="w-full flex items-center gap-2 px-2.5 py-1.5 text-[10px] text-foreground-muted hover:text-foreground hover:bg-background-subtle/50 rounded transition-colors">
                                  <Edit2 size={11} className="text-foreground-subtle shrink-0" /> Rename
                                </button>
                              )}
                              <button onClick={() => { toggleFavorite(file.id); setActiveMenuId(null); }} className="w-full flex items-center gap-2 px-2.5 py-1.5 text-[10px] text-foreground-muted hover:text-foreground hover:bg-background-subtle/50 rounded transition-colors">
                              ...
                              <button onClick={() => { alert(`Securely downloading: ${file.name}`); setActiveMenuId(null); }} className="w-full flex items-center gap-2 px-2.5 py-1.5 text-[10px] text-foreground-muted hover:text-foreground hover:bg-background-subtle/50 rounded transition-colors">
                              ...
                              {file.lockedBy ? (
                                ...
                                <button onClick={() => { deleteFile(file.id); setActiveMenuId(null); }} className="w-full flex items-center gap-2 px-2.5 py-1.5 text-[10px] text-destructive hover:bg-destructive/15 rounded transition-colors">
          ...
          <div className="w-full max-w-sm rounded-sm border border-border/80 bg-background-panel shadow-none p-5 space-y-4">
            ...
            <form onSubmit={handleUploadSubmit} className="space-y-3">
              ...
              <div className="flex items-center justify-end gap-2 text-[10px] font-bold font-mono pt-2">
                <button type="button" onClick={() => { setIsUploadModalOpen(false); setSelectedFile(null); }} className="h-8 px-3 rounded-sm border border-transparent bg-transparent text-foreground-subtle hover:text-foreground hover:bg-background-subtle/50 transition-colors">Cancel</button>
                <button type="submit" className="btn-shimmer h-8 px-4 rounded-sm font-mono text-[11px] font-bold uppercase tracking-wider text-accent-foreground">Commit Upload</button>
              </div>
            </form>
          </div>
        </div>
