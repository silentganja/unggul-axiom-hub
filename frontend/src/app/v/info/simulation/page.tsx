...
export default function InfoSimulationPage() {
  ...
  const steps: SimStep[] = [
    ...
    },
    {
      ...
      apiCall: "POST /api/webauthn/login/finish HTTP/1.1\nPayload: { \"challenge\": \"F8x7A29M_8b1...\", \"signature\": \"M28x...\" }\n\n<- Response: 200 OK\nSet-Cookie: session_token=JWT_PAYLOAD; HttpOnly; Secure; SameSite=Strict",
      sqlQuery: "SELECT public_key FROM user_credentials WHERE user_id = $1;",
      redisQuery: "GET challenge:user@unggul.axiom\nDEL challenge:user@unggul.axiom",
    },
    {
      ...
      apiCall: "POST /api/files/upload HTTP/1.1\nContent-Type: multipart/form-data\nFields: { \"parentId\": \"fold-102\", \"classification\": \"SULIT\" }\n\n<- Response: 201 Created\n{ \"id\": \"file-889\", \"name\": \"Q3_Budget.xlsx\", \"sizeBytes\": 409600 }",
      sqlQuery: "INSERT INTO files (name, type, parent_id, owner_id, classification, size_bytes)\nVALUES ($1, 'file', $2, $3, 'SULIT', $4)\nRETURNING id, name, classification;",
    ...
    },
    {
      ...
      apiCall: "POST /api/governance/requests HTTP/1.1\nPayload: { \"targetFileId\": \"file-889\", \"type\": \"CLASSIFICATION_UPGRADE\", \"newClassification\": \"RAHSIA\" }\n\n<- Response: 201 Created\n{ \"id\": \"req-552\", \"status\": \"PENDING\" }",
    ...
    },
    {
      ...
      apiCall: "POST /api/governance/requests/req-552/approve HTTP/1.1\nPayload: { \"reason\": \"Board review target confirmed\" }\n\n<- Response: 200 OK\n{ \"status\": \"approved\" }",
    ...
    },
    {
      ...
      sqlQuery: "INSERT INTO audit_logs (actor_id, action, target_id, ip_address)\nVALUES ($1, 'GOVERNANCE_APPROVE', $2, $3);\n/* Index matches actor_id and action for fast audit lookups */",
