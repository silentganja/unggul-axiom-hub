...
export default function FloatingActionBar() {
  ...
  return (
    <>
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-[calc(100%-2rem)] max-w-xl animate-in fade-in slide-in-from-bottom-4 duration-250 select-none">
        <div className="flex items-center justify-between gap-3 px-4 py-2.5 rounded-full border border-white/10 bg-background-panel/85 backdrop-blur-lg shadow-2xl shadow-black/50 text-foreground transition-all duration-300 hover:border-accent/35">
          <div className="flex items-center gap-2.5 shrink-0">
            <div className="h-6 w-6 rounded-full bg-accent/20 border border-accent/35 text-accent text-[11px] font-bold font-sans flex items-center justify-center shadow-sm shrink-0">
              {selectedIds.length}
            </div>
            <span className="font-sans text-[11px] font-bold tracking-wider text-foreground-subtle uppercase hidden xs:inline shrink-0">
          ...
          <div className="flex items-center gap-1.5 min-w-0">
            <button
              ...
              className="h-9 px-3 rounded-full border border-border/40 hover:border-accent/30 bg-background/30 hover:bg-accent/10 text-xs font-bold uppercase tracking-wider font-sans text-foreground-subtle hover:text-accent flex items-center gap-1.5 transition-all duration-200 group active:scale-95 cursor-pointer"
            >
              <Download size={12} className="text-accent group-hover:translate-y-[0.5px] transition-transform duration-200" />
              <span className="hidden sm:inline">Download</span>
            </button>
            <button
              onClick={() => { setIsMoveOpen(true); setMoveTargetId(currentFolderId); }}
              className="h-9 px-3 rounded-full border border-border/40 hover:border-accent/30 bg-background/30 hover:bg-accent/10 text-xs font-bold uppercase tracking-wider font-sans text-foreground-subtle hover:text-accent flex items-center gap-1.5 transition-all duration-200 group active:scale-95 cursor-pointer"
            >
              <FolderInput size={12} className="text-accent group-hover:translate-x-[0.5px] transition-transform duration-200" />
            ...
            <button
              ...
              className="h-9 px-3 rounded-full border border-border/40 hover:border-accent/30 bg-background/30 hover:bg-accent/10 text-xs font-bold uppercase tracking-wider font-sans text-foreground-subtle hover:text-accent flex items-center gap-1.5 transition-all duration-200 group active:scale-95 cursor-pointer"
            >
              <Shield size={12} className="text-accent group-hover:scale-105 transition-transform duration-200" />
              <span className="hidden sm:inline">Governance</span>
            </button>
            <span className="h-4 w-px bg-border/20 self-center" />
            <button
              onClick={deleteSelected}
              className="h-9 px-3 rounded-full border border-destructive/25 hover:border-destructive/40 bg-destructive/10 hover:bg-destructive/20 text-xs font-bold uppercase tracking-wider font-sans text-destructive flex items-center gap-1.5 transition-all duration-200 active:scale-95 cursor-pointer"
            >
              <Trash2 size={12} />
            ...
            <button
              ...
              className="h-8 w-8 rounded-full flex items-center justify-center bg-background/40 hover:bg-white/10 border border-border/40 text-foreground-subtle hover:text-foreground hover:rotate-90 transition-all duration-200 cursor-pointer"
